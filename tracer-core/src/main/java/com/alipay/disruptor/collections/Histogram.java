 ...
 */
package com.alipay.disruptor.collections;
...
public final class Histogram {
    ...
    private void validateBounds(final long[] upperBounds) {
        ...
        for (final long bound : upperBounds) {
            ...
            if (bound <= lastBound) {
                throw new IllegalArgumentException("bound " + bound + " is not greater than "
                                                   + lastBound);
            }
    ...
    public BigDecimal getMean() {
        ...
        for (int i = 0, size = upperBounds.length; i < size; i++) {
            if (0L != counts[i]) {
                ...
                BigDecimal intervalTotal = new BigDecimal(midPoint).multiply(new BigDecimal(
                    counts[i]));
