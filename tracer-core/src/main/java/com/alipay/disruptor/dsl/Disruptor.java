...
import com.alipay.disruptor.BatchEventProcessor;
...
import com.alipay.disruptor.EventTranslator;
import com.alipay.disruptor.EventTranslatorOneArg;
...
public class Disruptor<T> {
    ...
    @Deprecated
    public Disruptor(final EventFactory<T> eventFactory, final int ringBufferSize,
                     final Executor executor, final ProducerType producerType,
    ...
    public Disruptor(final EventFactory<T> eventFactory, final int ringBufferSize,
        ...
        this(RingBuffer.createMultiProducer(eventFactory, ringBufferSize), new BasicExecutor(
            threadFactory));
    }
     ...
     *
     * @param handlers the event handlers, previously set up with {@link #handleEventsWith(com.alipay.disruptor.EventHandler[])},
     ...
     *
     * @param processors the event processors, previously set up with {@link #handleEventsWith(com.alipay.disruptor.EventProcessor...)},
     *                   that will form the barrier for subsequent handlers or processors.
     ...
     *
     * @param eventTranslator the translator that will load data into the event.
     * @param <A>             the translator that will load data into the event.
    ...
    /**
     * Calls {@link com.alipay.disruptor.EventProcessor#halt()} on all of the event processors created via this disruptor.
     */
    public void halt() {
        for (final ConsumerInfo consumerInfo : consumerRepository) {
            consumerInfo.halt();
     ...
     * @see com.alipay.disruptor.Sequencer#getBufferSize()
     */
    ...
    EventHandlerGroup<T> createEventProcessors(final Sequence[] barrierSequences,
        ...
        for (int i = 0, eventHandlersLength = eventHandlers.length; i < eventHandlersLength; i++) {
            ...
            final BatchEventProcessor<T> batchEventProcessor = new BatchEventProcessor<T>(
    ...
    private void updateGatingSequencesForNextInChain(Sequence[] barrierSequences,
