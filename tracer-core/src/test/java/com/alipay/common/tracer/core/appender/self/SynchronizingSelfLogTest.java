...
public class SynchronizingSelfLogTest extends AbstractTestBase {
    ...
    public void error() throws IOException {
        ...
        File log = new File(TracerLogRootDaemon.LOG_FILE_DIR
                + File.separator + SynchronizingSelfLog.SELF_LOG_FILE);
        List<String> logs = FileUtils.readLines(log);
        assertTrue(logs.toString(), logs.get(0).contains("[ERROR]"));
        assertTrue(logs.toString(), logs.get(0).contains("test for SynchronizingSelfLog error situation"));
    ...
    public void error1() throws IOException {
        SynchronizingSelfLog.error("test for SynchronizingSelfLog error situation",new Throwable());
        SynchronizingSelfLog.flush();
        File log = new File(TracerLogRootDaemon.LOG_FILE_DIR
                + File.separator + SynchronizingSelfLog.SELF_LOG_FILE);
        List<String> logs = FileUtils.readLines(log);
